...
# time in minutes
def mile_run_walk_vo2(gender, time, bmi):
...
#  Kline et al. (1987)
#  for gender field, 1 for male, 0 for female
def mile_walk_vo2(gender, age, time, hr):
    if validate.male(gender):
        gender = 1.0
    elif validate.female(gender):
        gender = 0.0
...
def mile_half_run_vo2(gender, weight, time, hr=None):
    # male
    if validate.male(gender):
        gender = 1.0
...
# VO2 Max
def pop_vo2_max(gender, time, time2=None, time3=None):
    time = float(time)
    time2 = float(time2)
    time3 = float(time3)
    if validate.male(gender):
...
# grade (% incline) of treadmill in decimal form (e.g. 10% = 0.10)def walking_vo2(speed, grade):
    speed = float(speed)
    grade = float(grade)
...
# Running VO2
# speed of treadmill in meters / minute
# grade (% incline) of treadmill in decimal form (e.g. 10% = 0.10)def running_vo2(speed, grade):
    speed = float(speed)
    grade = float(grade)
...
# body mass in kilograms 1 kg = 2.2 lbdef leg_ergometry_vo2(work, mass):
    work = float(work)
    mass = float(mass)
...
# Arm Ergometry VO2
# work rate in kgm / min 1 Watt = 6 kgm / min
# body mass in kilograms 1 kg = 2.2 lbdef arm_ergometry_vo2(work, mass):
    work = float(work)
    mass = float(mass)
...
def stepping_vo2(height, frequency):
...
# 1 MET = 3.5 mL/kg/mindef vo2_reserve(max, rest):
    max = float(max)
    rest = float(rest)
    data = max - rest
    return data
...
# rest is Resting VO2
# reserve and rest must be of same unit type (METs or mL/kg/min)
# 1 MET = 3.5 mL/kg/mindef target_vo2(intensity, reserve, rest):
...
def residual_volume(gender, age, weight, height, bsa=None):
    ...
    elif validate.female(gender) and bsa:
        data = {
        "Berglund": (0.0115*age) + (0.019* height) - 2.24,
